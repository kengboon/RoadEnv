...
class PPO(object):
    ...
    def __init__(self, state_dim, action_dim, action_range=1., hidden_dim=128, method='clip'):
    ...
    def update(self):
        ...
        :return: None
        """
        s = Variable(torch.from_numpy(np.array(self.state_buffer)).float()).to(device) #torch.Tensor(self.state_buffer).to(device)
        a = Variable(torch.from_numpy(np.array(self.action_buffer)).float()).to(device) #torch.Tensor(self.action_buffer).to(device)
        r = Variable(torch.from_numpy(np.array(self.cumulative_reward_buffer)).float()).to(device) #torch.Tensor(self.cumulative_reward_buffer).to(device)
        with torch.no_grad():
    ...
    def finish_path(self, next_state, done):
        ...
        if done:
            ...
            next_state = Variable(torch.from_numpy(np.array([next_state])).float()).to(device)
            v_s_ = self.critic(next_state).cpu().detach().numpy()[0, 0]
        discounted_r = []
