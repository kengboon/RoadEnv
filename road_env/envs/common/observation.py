...
from road_env.envs.common.finite_mdp import compute_ttc_grid
...
class LidarKinematicsObservation(ObservationType):
    ...
    def normalize_obs(self, df: pd.DataFrame) -> pd.DataFrame:
        if not self.features_range:
            ...
            self.features_range = {
                ...
                "front_angle": self.observe_angle
            }
        for feature, f_range in self.features_range.items():
            if feature in df:
                df[feature] = utils.lmap(df[feature], [f_range[0], f_range[1]], [-1, 1])
    ...
    def angle_to_index(self, angle: float) -> int:
        return int(utils.lmap(angle, self.observe_angle, [0, self.cells-1]))
