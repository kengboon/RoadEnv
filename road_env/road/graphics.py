...
from road_env.road.lane import LineType, AbstractLane
...
from road_env.vehicle.objects import Obstacle, Landmark
...
class RoadObjectGraphics:
    ...
    def display(cls, object_: 'RoadObject', surface: WorldSurface, transparent: bool = False,
        ...
        """
        o = object_
        length = o.length if getattr(o, "length", None) else o.LENGTH
        width = o.width if getattr(o, "width", None) else o.WIDTH

        s = pygame.Surface((surface.pix(length), surface.pix(length)), pygame.SRCALPHA)  # per-pixel alpha
        rect = (0, surface.pix(length / 2 - width / 2), surface.pix(length), surface.pix(width))
    ...
    def get_color(cls, object_: 'RoadObject', transparent: bool = False):
        ...
        if isinstance(object_, Obstacle):
            if object_.crashed:
                # indicates failure
                color = cls.RED
            elif getattr(object_, "color", None):
                color = object_.color
            else:
                color = cls.YELLOW
